...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const produto = interaction.customId.replace(/^definir_valor_/, '').replace(/_\d+$/, '');

    const modal = new ModalBuilder()
      .setCustomId(`modal_valor_${produto}_${interaction.user.id}`)
      .setTitle('Definir Valor do Produto');

    const valorInput = new TextInputBuilder()
      .setCustomId('novo_valor')
      .setLabel('Valor do produto (ex: 19.90)')
      .setStyle(TextInputStyle.Short)
      .setPlaceholder('Digite o valor...')
      .setRequired(true);
