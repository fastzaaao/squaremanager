...
module.exports = {
  customId: /^ligar_app_(.+)_(\d+)$/, 
  async execute(interaction) {
    await interaction.deferUpdate();

    const match = interaction.customId.match(/^ligar_app_(.+)_(\d+)$/);
    if (!match) return;
  ...
  return interaction.followUp({
    embeds: [
      new EmbedBuilder()
        ...
        .setDescription(`# Mensalidade expirada ${getEmoji(emojis.data, '📦')}\n-# A mensalidade deste app expirou, você precisa renová-la para ligá-lo novamente.`)
    ],
    flags: 64
  });
}
