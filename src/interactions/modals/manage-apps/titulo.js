...
module.exports = {
  customId: /^modal_titulo_(.+)_\d+$/,
  async execute(interaction) {
    const customId = interaction.customId;
    const produto = customId.replace(/^modal_titulo_/, '').replace(/_\d+$/, '');
    const novoTitulo = interaction.fields.getTextInputValue('novo_titulo').trim();
    ...
    app.preco.embed.titulo = novoTitulo;
    ...
    await interaction.update({
      embeds: [
        new EmbedBuilder()
          .setColor(embedColor)
          .setDescription([
            `# Título atualizado ${getEmoji(emojis.lapis, '✏️')}`,
            `-# O título da embed do produto **${produto}** foi alterado para:`,
            `> \`${novoTitulo}\``
          ].join('\n'))
          .setThumbnail(interaction.user.displayAvatarURL({ dynamic: true })),
        produtoEmbed
