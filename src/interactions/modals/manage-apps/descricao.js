...
module.exports = {
  customId: /^modal_desc_(.+)_\d+$/,
  async execute(interaction) {
    const customId = interaction.customId;
    const produto = customId.replace(/^modal_desc_/, '').replace(/_\d+$/, '');
    const novaDescricao = interaction.fields.getTextInputValue('nova_descricao').trim();
    ...
    app.preco.embed.desc = novaDescricao;
    ...
    await interaction.update({
      embeds: [
        new EmbedBuilder()
          .setColor(embedColor)
          .setDescription([
            `# Descrição atualizada ${getEmoji(emojis.lapis, '✏️')}`,
            `-# A descrição da embed do produto **${produto}** foi alterada para:`,
            `> ${novaDescricao}`
          ].join('\n'))
          .setThumbnail(interaction.user.displayAvatarURL({ dynamic: true })),
        produtoEmbed
